...
// Async thunk to check if phone exists
...
export const updateCustomerDetails = createAsyncThunk<
    ...
    { rejectValue: string }
>("customer/updateCustomerDetails", async ({ phone, name, tableNumber }, { rejectWithValue }) => {
    try {
        await setDoc(doc(collection(db, "customers"), phone), { name }, { merge: true });
        await setDoc(doc(collection(db, "orders"), tableNumber), { tableNumber, phone });
        return { success: true };
    } catch (error) {
        return rejectWithValue((error as Error).message);
    }
